...
# 5. Unit Handover Item Model (models/unit_handover_item.py)
class UnitHandoverItem(models.Model):
    _name = 'unit.handover.item'
    _description = 'Unit Handover Item'

    form_id = fields.Many2one('unit.handover.form', string='Form')
    item_name = fields.Char(string='Item Name', required=True)
    description = fields.Text(string='Description')
    checked = fields.Boolean(string='Available')

# 6. Add One2many in unit.handover.form (models/unit_handover_form.py)
class UnitHandoverForm(models.Model):
    _name = 'unit.handover.form'
    _description = 'Unit Handover Form'
    ...
    handover_item_ids = fields.One2many(
        'unit.handover.item',
        'form_id',
        string='Items'
    )
